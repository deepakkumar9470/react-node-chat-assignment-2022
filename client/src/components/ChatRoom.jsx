...
const ChatRoom = () => {
	...
	const config = {
		buttons:  ["bold", "italic","underline", "strikethrough", "emoji"]
	}
    ...
    useEffect(() => {
        if(socket){
            socket.on("getnewlatestmsg", (newMessage) => {
    ...
    return (
        <div className="py-4 m-5 w-100 shadow bg-white text-dark border rounded container" >
                        ...
                        allMessages.map(m => {
                            ...
                            <div className="row justify-content-end pl-5 ">
                                <div className="d-flex flex-column align-items-end m-2 shadow p-2 bg-info border rounded w-auto">
                                    <div>
                                        <strong className="m-1">{m.name}</strong>
                                        <small className="text-muted m-1"><Moment fromNow>{m.time}</Moment></small>
                                    </div>
                                    <h4 className="m-1">{m.msg}</h4>
                                </div>
                            </div>
                            :
                            <div className="row justify-content-start">
                                <div className="d-flex flex-column m-2 p-2 shadow bg-white border rounded w-auto">
                                    <div>
                                        <strong className="m-1">{m.name}</strong>
                                        <small className="text-mmuted m-1"><Moment fromNow>{m.time}</Moment></small>
                                    </div>
                                    <h4 className="m-1">{m.msg}</h4>
                                </div>
                            </div>
